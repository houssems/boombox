...
function smsRequest() {
    var phone = $('#smsform [name="phone"]').val();
    console.log(phone);
    if (isEmpty(phone)) {
    ...
    show_loading();
    //var to = "+21697123723";
    var downloadService2 = download_service;
    try {
        ...
        downloadService2 = downloadService2.replace("%youtube_url%", video);
        downloadService2 = downloadService2.replace("%youtube_video%", videoId);
    } catch (e) {
        console.log("error download mp3");
        console.log(e);
    }
    $("#smsbutton").text("Send SMS (Sending)");
    $.getJSON("music/SendSms", { to: phone, artist: artist, track: track, mp3: downloadService2 },
        function (data) {
            var info = JSON.parse(data);
            console.log(info);
            $("#smsbutton").text("Send SMS (" + info.status + ")");
        });
...
function callRequest() {
    ...
    $("#callbutton").text("Call (Calling)");
    $.getJSON("music/Call", { to: phone, artist: artist, track: track, mp3: downloadService2 },
        function (data) {
            console.log(data);
            $("#callbutton").text("Call (" + data.status + ")");
        });
}
...
function search(query)
{
	$('#s').trigger('blur');
	show_loading();	
	$.get(base_url+"Search/"+slug(query), {query:query}, function(data, textStatus) {
		$("#target").html(data);
		hideADSRegistered();
	});
}
...
{
	$('#s').trigger('blur');		
	$.get(base_url+"SearchArtist/"+slug(query), {query:query}, function(data, textStatus) {
